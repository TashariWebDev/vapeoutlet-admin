<div wire:poll.5000ms="$refresh">
  <div>
    <x-modal x-data="{ show: $wire.entangle('quickViewCustomerAccountModal') }">
      <div class="pb-3">
        <h3 class="text-2xl font-bold text-slate-600 dark:text-slate-300">Latest
                                                                          transactions</h3>
      </div>
          ...
          @if ($selectedCustomerLatestTransactions)
            <div class="py-6">
              <div class="hidden grid-cols-4 gap-2 py-3 px-1 lg:grid">
                <div class="text-xs font-semibold text-left text-slate-600 dark:text-slate-300">
                  TYPE
                </div>
                <div class="text-xs font-semibold text-left text-slate-600 dark:text-slate-300">
                  REF
                </div>
                <div class="text-xs font-semibold text-right text-slate-600 dark:text-slate-300">
                  AMOUNT
                </div>
                <div class="text-xs font-semibold text-right text-slate-600 dark:text-slate-300">
                  BAL
  ...
  <div class="px-4 bg-white rounded-md shadow-sm dark:bg-slate-900">
    ...
    <div class="grid grid-cols-1 gap-y-4 py-3 lg:grid-cols-6 lg:gap-x-3 lg:px-0">
      <div>
        <label for="">
      ...
      <div>
        <x-input.select wire:model="customerType"
                        label="Order Type"
        >
      ...
      <div>
        <label for="fromDate">
          <x-input.label>From Date</x-input.label>
          <x-input.text
              type="date"
              wire:model="fromDate"
              name="fromDate"
      ...
      <div>
        <label for="toDate">
          <x-input.label>To Date</x-input.label>
          <x-input.text
              type="date"
              wire:model="toDate"
              name="toDate"
  ...
  @if($filter === 'abandoned')
    ...
    <div class="py-1 px-2 text-gray-500 dark:text-gray-300 text-[10px]">
      ...
      <p>
        Cancelling the order moves the order under the cancelled tab and can not be edited
      </p>
      <p>
        To accept the order, click on the order, click edit, and continue to process as normal.
      </p>
    </div>
          ...
          <div class="absolute top-0 mt-10 w-auto h-auto text-white rounded-md shadow-lg origin-top bg-slate-800">
            <div class="py-2">
              ...
              <div class="mb-2">
                <button class="py-2 px-2 w-full font-semibold text-left uppercase text-[10px] hover:bg-slate-900"
                        onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                        wire:click="printInvoicesInBulk"
                >
                  Batch Download
                </button>
              </div>
  ...
  <div>
    {{-- Desktop --}}
    <div class="pb-2">
      <div class="mt-4 rounded-md shadow lg:bg-white dark:bg-slate-900">
        <x-table.container>
          ...
          <div>
            <div>
              @forelse($orders as $order)
                <div wire:key="orders_{{ $order->id }}">
                  <div>
                    <x-table.body class="hidden grid-cols-1 lg:grid lg:grid-cols-6">
                      ...
                      <x-table.row class="px-2 text-center cursor-default lg:text-left text-semibold dark:text-slate-400">
                        ...
                        >
                          {{ $order?->delivery?->description ?? 'Delivery Not set' }}
                        </p>
                        <p
                    ...
                    <div class="grid grid-cols-1 gap-y-4 mt-3 shadow-sm lg:hidden">
                      <div class="grid grid-cols-2 px-2 mt-2 bg-white rounded-md shadow-sm dark:bg-slate-800">
                        ...
                        <div class="flex-col col-span-2 pb-2 mt-3 space-y-2 w-full">
                          @if(is_null($order->status))
                            <div>
                              <livewire:orders.cancel-abandoned
                                  :order="$order"
