<div>
    <div class="overflow-hidden bg-white shadow sm:rounded-lg dark:bg-slate-800">
        ...
        <div class="border-t border-slate-200">
            <dl>
                <div class="py-5 px-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 bg-slate-50 dark:bg-slate-900">
                    <dt class="text-sm font-medium text-slate-500 dark:text-slate-400">Full name</dt>
                    <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0 text-slate-900 dark:text-slate-300">{{ ucwords($customer->name) }}</dd>
                </div>
                ...
                <div class="py-5 px-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 bg-slate-50 dark:bg-slate-900">
                    ...
                    <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0 text-slate-900 dark:text-slate-300">
                        @foreach($customer->addresses as $address)
                            <div class="py-2">
                                <p>
                                    {{ ucwords($address->line_one) }} {{ ucwords($address->line_two) }} {{ ucwords($address->suburb) }}
                                    {{ ucwords($address->city) }} {{ ucwords($address->province) }} {{ $address->postal_code }}
                                </p>
                            </div>
                        @endforeach
                    </dd>
                ...
                <div class="py-5 px-4 bg-white sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 dark:bg-slate-700">
                    ...
                    <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0 text-slate-900 dark:text-slate-300">
                        <ul role="list"
                            class="rounded-md border divide-y border-slate-200 divide-slate-200"
                        >
                            @if($customer->id_document)
                                <li class="flex justify-between items-center py-3 pr-4 pl-3 text-sm">
                                    <div class="flex flex-1 items-center w-0">
                                        ...
                                        <svg class="flex-shrink-0 w-5 h-5 text-slate-400"
                                    ...
                                    <div class="flex-shrink-0 ml-4">
                                        <a href="{{ config('app.frontend_url') .'/storage/'.$customer->id_document }}"
                            ...
                            @if($customer->cipc_documents)
                                <li class="flex justify-between items-center py-3 pr-4 pl-3 text-sm">
                                    ...
                                    <div class="flex-shrink-0 ml-4">
                                        <a href="{{ config('app.frontend_url') .'/storage/'.$customer->cipc_documents }}"
                                           class="font-medium text-green-600 hover:text-green-500"
                                           download
                                        >Download</a>
                                    </div>
        ...
        <div class="container grid grid-cols-2 gap-6 p-6 mx-auto w-full bg-white rounded-lg lg:grid-cols-4 dark:bg-slate-800">
            @foreach($customer->businessImages as $image)
                <div class="w-full bg-white rounded-lg shadow">
                    <img src="{{ config('app.frontend_url') .'/storage/'.$image->photo }}"
                         alt="image"
