...
<head>
  ...
  >
  <title>{{ ucwords(str_replace('Admin','',config('app.name'))) }} {{ $brand }} Products sales by
                                                                                volume | {{ $from }}
                                                                                - {{ $to }}
  </title>
      ...
      <div class="px-4">
        <table class="w-full">
          ...
          <tbody class="text-[8px]">
            ...
            @php
              $noOfDays = Carbon\Carbon::parse($to)->diffIndays(Carbon\Carbon::parse($from));
            @endphp
              ...
              @foreach ($products as $product)
                ...
                <tr class="py-1 border-b border-dashed break-inside-avoid-page text-[8px]">
                  <td
                      ...
                      colspan="2"
                  >
                    <p class="font-bold">{{ $product->brand }} {{ $product->name }} {{ $product->category }}</p>
                    <p>{{ $product->sku }}</p>
