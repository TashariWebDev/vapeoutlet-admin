...
<div
    class="relative border rounded-md px-2 py-2 ">
    <label for="{{$label}}"
           class="absolute -top-2 left-1 -mt-px inline-block px-1 bg-white text-xs font-medium text-gray-900">
        {{ $label ?? '' }}
    </label>
    <select id="{{$label}}" {{ $attributes }}
    class="block w-full rounded-md border-none outline-0 p-0 text-gray-900 placeholder-gray-500 ring-gray-500 sm:text-sm"
    >
